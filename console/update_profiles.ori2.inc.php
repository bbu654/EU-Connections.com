...
function release_profile( $_POST , $tablepre = "euconnect_" ){
	// function to process the form results into a SQL query
	// to update the release field specifically
	$release_sql = "UPDATE " . $tablepre . "profiles SET " ;
	$release_sql .= " released = " . addslashes ( $_POST["released"] ) . " " ;
	$release_sql .= " WHERE prof_id = \"" . $_POST["prof_id"] . "\" " ;
	//	echo $update_sql ;
	$result = mysql_query( $release_sql ) or die( mysql_error() ) ;
	echo "<div align=\"center\"><h4>Profile " . $_POST["priority"] . " released successfully.</h4></div>" ;
	return $result ;
}
...
function delete_profile( $_POST , $tablepre = "euconnect_" ){
	...
	echo "<div align=\"center\"><h4>Profile " . $_POST["priority"] . " deleted successfully.</h4></div>" ;
	return $result ;
}

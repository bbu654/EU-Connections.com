...
function write_profilesheet( &$result , $select = "active" ,  $sheetnum = 2 , $user_adminlist = 0 ) {
	switch( $select ){
		case "active" :
   	...
   	$worksheet = " <Worksheet ss:Name=\"EU " . $selectext . " Profiles\">\n" ; 
	$printrows = ( mysql_num_rows( $result ) ) ? mysql_num_rows( $result ) + 5 : 5 ;  	
   	$worksheet .= "  <Names>\n" ;
   	$worksheet .= "   <NamedRange ss:Name=\"Print_Area\" ss:RefersTo=\"=Sheet" . $sheetnum . "!R1C1:R" . $printrows . "C12\"/>\n" ;
   	$worksheet .= "   <NamedRange ss:Name=\"Print_Titles\" ss:RefersTo=\"=Sheet" . $sheetnum . "!R1:R3\"/>\n" ;
   	$worksheet .= "  </Names>\n" ;
...
function write_membersheet( &$result , $sheetnum = 1 , $user_adminlist = 0 ) {
   	$worksheet = " <Worksheet ss:Name=\"Active Members\">\n" ; 
	$printrows = ( mysql_num_rows( $result ) ) ? mysql_num_rows( $result ) + 5 : 5 ;  	
   	$worksheet .= "  <Names>\n" ;
   	$worksheet .= "   <NamedRange ss:Name=\"Print_Area\" ss:RefersTo=\"=Sheet" . $sheetnum . "!R1C1:R" . $printrows . "C12\"/>\n" ;
   	$worksheet .= "   <NamedRange ss:Name=\"Print_Titles\" ss:RefersTo=\"=Sheet" . $sheetnum . "!R1:R3\"/>\n" ;
...
function write_alumnisheet( &$result , $sheetnum = 3 , $user_adminlist = 0 ) {
   	$worksheet = " <Worksheet ss:Name=\"EU Alumni\">\n" ; 
	$printrows = ( mysql_num_rows( $result ) ) ? mysql_num_rows( $result ) + 5 : 5 ;  	
   	$worksheet .= "  <Names>\n" ;
   	$worksheet .= "   <NamedRange ss:Name=\"Print_Area\" ss:RefersTo=\"=Sheet" . $sheetnum . "!R1C1:R" . $printrows . "C10\"/>\n" ;
   	$worksheet .= "   <NamedRange ss:Name=\"Print_Titles\" ss:RefersTo=\"=Sheet" . $sheetnum . "!R1:R3\"/>\n" ;
   	$worksheet .= "  </Names>\n" ;
   	$worksheet .= "  <Table x:FullColumns=\"1\" x:FullRows=\"1\">\n" ;
   	$worksheet .= "   <Column ss:AutoFitWidth=\"0\" ss:Width=\"125\"/>\n" ;
...
if ( mysql_num_rows( $result ) ) {
	echo write_membersheet( $result , 1 , $user_admin ) ;
}

$sql_list = profile_select_query( "active" , $tablepre ) ;
$result = mysql_query( $sql_list ) or die( mysql_error() ) ;
if ( mysql_num_rows( $result ) ) {
	echo write_profilesheet( $result , "active" , 2, $user_admin ) ;
}

$sql_list = member_select_query( "alumni" , $tablepre ) ;
$result = mysql_query( $sql_list ) or die( mysql_error() ) ;
// if ( mysql_num_rows( $result ) ) {
	echo write_alumnisheet( $result , 3 , $user_admin ) ;
// }

$sql_list = profile_select_query( "active" , $tablepre ) ;
$result = mysql_query( $sql_list ) or die( mysql_error() ) ;
if ( mysql_num_rows( $result ) ) {
//	echo write_profilesheet( $result , "alumni" , 3 , $user_admin ) ;
}
